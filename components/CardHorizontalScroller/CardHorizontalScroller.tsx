...
> {
  ...
  handleScroll(direction: number) {
    ...
    const canScrollRight =
      futureScrollLeft < this.node.scrollWidth - this.node.clientWidth;
    this.setState({
      canScrollLeft,
      canScrollRight
    });
  ...
  render() {
    ...
    return (
      <HorizontalScrollBarContainer {...others}>
        {this.state.canScrollLeft && (
          <ChevronStyles onClick={() => this.handleScroll(-1)}>
            <ChevronLeft />
          </ChevronStyles>
        )}
        ...
        {this.state.canScrollRight && (
          <ChevronStyles right={true} onClick={() => this.handleScroll(1)}>
            <ChevronRight />
          </ChevronStyles>
        )}
